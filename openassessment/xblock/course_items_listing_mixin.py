...
class CourseItemsListingMixin:
    ...
    def download_ora2_responses(self, request, suffix=''):  # pylint: disable=unused-argument
        from openassessment.data import OraAggregateData
        ora_responses = OraAggregateData.collect_ora2_responses(six.text_type(self.course_id))
        ...
        with store.bulk_operations(course_key):
            ...
            for block in openassessment_blocks:
                result_item_id = six.text_type(block.location.replace(version_guid=None, branch=None))
